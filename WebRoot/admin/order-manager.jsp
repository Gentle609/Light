				...
				<tbody>
					...
					<tr>
						...
						<td class="td-status"><span
							class="label label-success radius"><%=o.getOrderStatus()%></span></td>
						<%
							if (o.getOrderStatus().equals("已提交")) {
						%>
						<td><a href="../OrderAdminSendServlet?id=<%=o.getoID()%>"
							title="发货" class="btn btn-xs btn-success"
							onclick="if(confirm('确定要发货吗？')) return true;else return false;"><i
								class="fa fa-cubes bigger-120"></i></a></td>
						<%
							}else{
						%>
						<td><a class="btn btn-xs btn-success">无</a></td>
						<%
							}
						%>
					</tr>
					...
					<tr>
						<td colspan="6">
							...
							<div class="page">
								<a href="order-manager.jsp?page=1">首页</a> <a
									href="order-manager.jsp?page=<%=curPage - 1 > 0 ? curPage - 1 : 1%>">上一页</a>
								<a
									href="order-manager.jsp?page=<%=curPage + 1 < pageTotal ? curPage + 1 : pageTotal%>">下一页</a>
								<a href="order-manager.jsp?page=<%=pageTotal%>">尾页</a>
							</div>
						</td>
					</tr>
					<%
